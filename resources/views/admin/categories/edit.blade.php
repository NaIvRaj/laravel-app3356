...
<div class="bg-white p-8 rounded-3xl border border-slate-100 shadow-sm max-w-xl">
    <form action="{{ route('admin.categories.update', $category->id) }}" method="POST">
        ...
        <div class="mb-6">
            ...
            <input type="text" name="name" value="{{ $category->name }}" class="w-full px-5 py-4 border-2 border-slate-100 rounded-2xl outline-none focus:border-indigo-600 transition" required>
        </div>
